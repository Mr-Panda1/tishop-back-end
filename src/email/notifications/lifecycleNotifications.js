...
async function sendSellerProductDeletedByAdminEmail({
  toEmail,
  sellerName,
  productName,
  reason,
  ...
  const reasonBlock = reason
    ? `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
        <tr>
          <td style="padding:12px 16px;background-color:#fff7ed;border-radius:6px;">
            <p style="margin:0 0 4px;font-size:13px;font-weight:600;color:#9a3412;">Raison</p>
            <p style="margin:0;font-size:13px;color:#5c6370;line-height:1.5;">${escapeHtml(reason)}</p>
          </td>
        </tr>
      </table>`
    : '';

  const safeProductName = productName || 'Produit';
  const subject = `${safeProductName} — supprimé par TiShop`;
  const html = `<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:8px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:0 32px 32px;">
              <h1 style="margin:0 0 8px;font-size:22px;font-weight:600;color:#1a1d24;text-align:center;">Produit retiré de la plateforme</h1>
              <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#5c6370;text-align:center;">Bonjour ${escapeHtml(sellerName || 'vendeur')}, votre produit <strong style="color:#1a1d24;">${escapeHtml(safeProductName)}</strong> a été supprimé par l'équipe TiShop.</p>
              ${reasonBlock}
              <p style="margin:0 0 20px;font-size:13px;line-height:1.6;color:#5c6370;text-align:center;">Si vous pensez qu'il s'agit d'une erreur, vous pouvez contacter le support à <a href="mailto:support@tishop.co" style="color:#7c3aed;text-decoration:none;">support@tishop.co</a>.</p>
            </td>
          </tr>
          <tr>
            <td style="padding:20px 32px;border-top:1px solid #ebebef;text-align:center;">
...
module.exports = {
  ...
  sendSellerProductDeletedByAdminEmail,
  statusLabel
};
